  ...
  for (const key of Object.keys(fragment)) {
    ...
    if (key in target) {
      ...
      if (typeof specVal !== typeof fragVal) {
        ...
        throw new Error(
          `Attempted to merge ${JSON.stringify(fragVal)} into incompatible ${JSON.stringify(
            specVal,
          )} at path ${jsonPath.join('/')}/${key}`,
        );
      }
      if (specVal == fragVal) {
        continue;
      ...
      if (typeof specVal !== 'object') {
        // eslint-disable-next-line max-len
        throw new Error(
          `Conflict when attempting to merge ${JSON.stringify(fragVal)} into ${JSON.stringify(
            specVal,
          )} at path ${jsonPath.join('/')}/${key}`,
        );
      }

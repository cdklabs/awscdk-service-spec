...
function matchTypeFieldsToResources(resources: Resource[], typeInfos: EventTypeDefinition[]): ResourceMatch[] {
  ...
  for (const resource of resources) {
    ...
    const resourceName = praseResourceName(resource.cloudFormationType);

    for (const typeFieldName of typeInfos) {
      const typeSegment = normalizeName(typeFieldName.name);

      if (typeSegment == resourceName.toLowerCase()) {
        matches.push({
      ...
      for (const propertiesFieldName of Object.keys(typeFieldName.properties)) {
        ...
        if (fieldSegment == resourceName.toLowerCase()) {
          matches.push({
            type: ref(typeFieldName),
    ...
    if (matches.length > 0) {
      if (matches.length > 1) {
        // 17 events affected by this, some of them has resourceId & resourceName, some has in multiple levels the resource
...
/**
 *  Resource CloudFromation type e.g. "AWS::S3::Bucket" returns "Bucket"
 */
function praseResourceName(resourceCFType: string) {
  const parts = resourceCFType.split('::');
  return parts[2];
}

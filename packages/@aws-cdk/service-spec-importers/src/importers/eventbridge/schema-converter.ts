...
export function createTypeDefinitionsFromSchema(
  ...
  if (isEmptyObjectType(detailSchema)) {
    ...
    const { eventTypeDefinitionBuilder } = ctx.specBuilder.eventTypeDefinitionBuilder(detailTypeName, {
      schema: detailSchema,
    });
...
export function schemaTypeToModelType(
  ...
  return tryCatch(fail, (): Result<PropertyType> => {
    ...
    // const nameHint = referenceName ? lastWord(referenceName) : propertyName;
...
function objectLikeSchemaToModelType(
  ...
  const { eventTypeDefinitionBuilder, freshInSession } = ctx.specBuilder.eventTypeDefinitionBuilder(nameHint, {
    schema,
  });
